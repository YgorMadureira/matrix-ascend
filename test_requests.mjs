...
async function test() {
  console.log("Checking training_scheduling_requests schema...");
  const res = await supabase.from('training_scheduling_requests').select('*').limit(1);
  console.log("Requests table result:", res);
}
