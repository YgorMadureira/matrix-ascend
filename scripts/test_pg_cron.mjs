...
async function test() {
  console.log("Checking database connection and extensions...");
  const { data, error } = await supabase.from('collaborators').select('count', { count: 'exact', head: true });
  console.log("Collaborators count:", data, error);
}
